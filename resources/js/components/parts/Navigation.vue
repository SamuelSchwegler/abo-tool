        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6">
            <div class="flex justify-between items-center border-b-2 border-gray-100 py-4 md:justify-start md:space-x-10">
                ...
                <PopoverGroup as="nav" class="hidden md:flex space-x-10" v-if="isLoggedIn">
                    ...
                    <router-link to="/my-orders" class="text-base font-medium text-gray-500 hover:text-gray-900"> Meine Lieferungen </router-link>
                            ...
                            <PopoverPanel class="absolute z-10 left-1/2 transform -translate-x-1/2 mt-3 px-2 w-screen max-w-md sm:px-0">
                                <div class="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden">
                                    <div class="relative grid gap-6 bg-white px-5 py-6 sm:gap-8 sm:p-8">
                                        <a v-for="item in settings" :key="item.name" :href="item.href" class="-m-3 p-3 flex items-start rounded-lg hover:bg-gray-50">
                ...
                <div class="hidden md:flex items-center justify-end md:flex-1 lg:w-0" v-if="isLoggedIn">
                    <a style="cursor: pointer;" @click="logout" class="ml-8 whitespace-nowrap inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-violet hover:bg-indigo-700"> Logout </a>
                </div>
                <div class="hidden md:flex items-center justify-end md:flex-1 lg:w-0" v-else>
                    <router-link to="/login" class="whitespace-nowrap text-base font-medium text-gray-500 hover:text-gray-900"> Login </router-link>
                    <router-link to="/register" class="ml-8 whitespace-nowrap inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-indigo-600 hover:bg-indigo-700"> Registrieren </router-link>
                </div>
            </div>
        </div>
                    ...
                    <div class="pt-5 pb-6 px-5">
                        ...
                        <div class="mt-6">
                            <nav class="grid gap-y-8">
                                <a v-for="item in settings" :key="item.name" :href="item.href" class="-m-3 p-3 flex items-center rounded-md hover:bg-gray-50">
                                    <component :is="item.icon" class="flex-shrink-0 h-6 w-6 text-indigo-600" aria-hidden="true" />
