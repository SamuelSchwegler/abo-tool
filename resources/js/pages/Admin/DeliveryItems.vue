    ...
    <div class="max-w-md mx-auto sm:max-w-3xl">
        <div>
            <div class="text-center">
                ...
                <p class="mt-1 text-sm text-gray-500" v-if="items.length === 0">Momentan wurden noch keine Produkte der Lieferung hinzugefügt</p>
            </div>
            <div class="mt-6 sm:flex sm:items-center">
                ...
                <div class="relative rounded-md shadow-sm sm:min-w-0 sm:flex-1">
                    <search-select :items="allItems" :key="'item-search_' + itemSearchKey"></search-select>
                </div>
                <div class="mt-3 sm:mt-0 sm:ml-4 sm:flex-shrink-0">
                    <button @click="addItem(2)"
...
export default {
    ...
    methods: {
        async addItem(item_id) {
            await axios.post('/api/delivery/' + this.$route.params.id + '/item/' + item_id).then(response => {
